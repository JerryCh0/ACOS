...
int main()
{
	while (1){
		...
		for (int i = 0; i <= qty_cmd; ++i)
			if (i == qty_cmd || strcmp(arr_cmd[i], "|") == 0){
				...
				if (pid == 0){ //если мы в процессе-потомке
					if (i != qty_cmd){ //если мы работаем с последним словом в строке, то переменная для хранения ФД, предназаначенного для input следующей команды, так как следующей команды уже не будет
					...
					fd = pipefd[1];
					if (i == qty_cmd && buf1[0]){ //если мы в конце строки, и есть перенаправление вывода (если в буфере что-то написано)
		...
		for (int i = 0; i < qty_cmd; ++i) //в конце освобождаем память
			free(arr_cmd[i]);
